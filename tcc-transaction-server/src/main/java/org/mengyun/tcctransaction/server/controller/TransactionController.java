...
import org.mengyun.tcctransaction.server.dao.JdbcIdempotentDao;
...
public class TransactionController {
	...
	@Autowired
	private JdbcIdempotentDao jdbcIdempotentDao;
	...
	public CommonResponse<Void> reset(@PathVariable String domain, String globalTxId, String branchQualifier) {
		...
		transactionDao.resetRetryCount(domain, DatatypeConverter.parseHexBinary(globalTxId), DatatypeConverter.parseHexBinary(branchQualifier));

		return new CommonResponse<Void>();
	}

	@RequestMapping(value = "/idempotent/{gtxid}", method = RequestMethod.GET)
	@ResponseBody
	public CommonResponse<List<IdempotentVo>> idempotentList(@PathVariable String gtxid) {
		List<IdempotentVo> ret = jdbcIdempotentDao.findIdempotent(DatatypeConverter.parseHexBinary(gtxid));
		return new CommonResponse<List<IdempotentVo>>(200, "success", ret);
	}
